...
const useStyles = createUseStyles({
    activeContainer: {
        backgroundColor: ({ theme }) => theme.color.paleGreenTransparent
    },
    container: {
        display: 'flex',
        height: 56,
    ...
    leftBar: {
        borderLeft: ({ theme, level }) =>
            level > 1 ? 'none' : `3px solid ${theme.color.darkGreenishVibe}`
    },
    title: {
        fontSize: 16,
        lineHeight: '20px',
        letterSpacing: '0.2px',
        color: ({ theme, isActive }) => (isActive ? theme.color.paleGreen : theme.color.greenishVibe),
        marginLeft: 24
    }
});
...
function MenuItemComponent({ children, icon: Icon, id, items = [], level = 1, onClick, title }) {
    ...
    const iconColor = isActive ? theme.color.paleGreen : theme.color.greenishVibe2;
